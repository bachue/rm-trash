...
## 安装
将项目文件下载后存储在能够被用户访问到的任意目录，在`~/.zshrc`或是`~/.bashrc`或是任意初始化时会执行的shell脚本中编写一个shell函数`rm`，用来调用项目文件中的`rm.rb`文件，并将所有参数一并传入，例如：

```
rm () {
	$ZSH/bin/rm-trash/rm.rb $@
}
```
...
* 实现了`--rm`选项，将从`$PATH`中按顺序搜索`rm`文件并执行。（_这种做法在大部分情况下会调用到系统内置的`rm`命令，对吧_）
